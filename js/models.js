...
class User {
  ...
  //is owns story
  isOwnStory(story){
    return this.ownStories.some(own => {
  ...
  async removeFavoriteStory(story){
    ...
    const favoriteIdex = this.favorites.findIndex(fav => {
      return fav.storyId === story.storyId
    })

    this.favorites.splice(favoriteIdex,1)
  }

  //display favorite story
  handleUserFavorite(){
  ...
  async deleteMyStory(story){
    ...
    const deleteIdex = this.ownStories.findIndex(own =>{
      return own.storyId === story.storyId
    })

    this.ownStories.splice(deleteIdex,1)
  }
}
